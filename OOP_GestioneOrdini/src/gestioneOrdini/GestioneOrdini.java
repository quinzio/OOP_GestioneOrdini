...
public class GestioneOrdini {
    ...
    List<LineaOrdineCliente> lineeordine = new ArrayList<>();
	...
	try (Scanner scanner = new Scanner(prodotti)) {
		...
		f.addProdotto(this.prodotti.get(pr));
	    }
	} catch (Exception e) {
	    System.out.println("Problem scanning during ordine fornitore");
	}
     ...
     * univoco, quindi è segnalata la duplicazione. Un ordine cliente si
     * compone di linee, una per prodotto; i prodotti sono distinti (altrimenti
     * è segnalata la duplicazione). Ciascuna linea punta al prodotto di cui è
     ...
     **/
    public void addOrdineCliente(String codice, String cliente, String prodotti) throws Exception {
	...
	try (Scanner scanner = new Scanner(prodotti)) {
	    ...
	    System.out.println("Scanner problems");
	}
	OrdineCliente oc = new OrdineCliente(cliente);
	ordiniCliente.put(codice, oc);
	try (Scanner scanner = new Scanner(prodotti)) {
	    scanner.useDelimiter(",");
	    while (scanner.hasNext()) {
		String p = scanner.next();
		LineaOrdineCliente loc = new LineaOrdineCliente(cliente, codice, this.prodotti.get(p));
		oc.addLinea(loc);
		lineeordine.add(loc);
	...
	if (!ordiniCliente.containsKey(codice))
	    throw new Exception();
	return ordiniCliente.get(codice).getLinee().stream().map(LineaOrdineCliente::getProdotto)
		.collect(Collectors.summingInt(Prodotto::getPrice));
    }

    /**
     * Il metodo void addOrdineFornitore (String codice, String fornitore,
     * String linee) throws Exception inserisce un ordine fornitore; il codice
     * è univoco, quindi è segnalata la duplicazione.
     * g.addOrdineFornitore("alfa:1", "AlfaMobili", "quercia:1 divano2pT,
     * faggio:1 poltronaP"); //importo = 450 Un ordine fornitore aggrega linee
     * di ordini cliente: ogni linea è indicata dal codice dell'ordine cliente
     * e dal nome del prodotto. L'ordine fornitore mostrato aggrega quindi la
     * linea divano2pT dell'ordine cliente quercia:1 e la linea poltronaP
     * dell'ordine cliente faggio:1. Il metodo lancia un'eccezione se il
     * fornitore non fornisce i prodotti corrispondenti alle linee e se le linee
     * indicate sono inesistenti (errore nel codice ordine cliente o nel nome
     * del prodotto) o sono già state aggregate in un altro ordine fornitore.
     * 
     * 
     **/
    public void addOrdineFornitore(String codice, String fornitore, String linee) throws Exception {

	OrdineFornitore of = new OrdineFornitore(codice, fornitore);
	ordiniFornitore.put(codice, of);
	...
	try (Scanner scanner = new Scanner(linee)) {
		...
		LineaOrdineCliente loc = ordiniCliente.get(codiceOrdine).getLinee().stream()
			.filter(l -> l.getProdotto().getName().equals(nomeProdotto)).findFirst().get();
	...
	return ordiniCliente.values().stream().filter(o -> o.isStatoconsgenato()).map(OrdineCliente::getName).sorted()
	...
	return ordiniCliente.values().stream()
		.collect(Collectors.groupingBy(OrdineCliente::getName, TreeMap::new, Collectors.counting()));
    }
	...
	Optional<Long> ol = ordiniCliente.values().stream().map(OrdineCliente::getSomma)
	...
	return ordiniCliente.values().stream()
		.collect(Collectors.groupingBy(OrdineCliente::getName,
			Collectors.<OrdineCliente>summingInt(o -> Math.toIntExact(o.getSomma()))))
		.entrySet().stream().collect(Collectors.groupingBy(e -> e.getValue(), () -> {
		    return new TreeMap<Integer, TreeSet<String>>(Comparator.reverseOrder());
	...
	return lineeordine.stream()
		.collect(Collectors.groupingBy(l -> l.getProdotto().getName(), Collectors.counting())).entrySet()
		.stream().collect(Collectors.groupingBy(e -> e.getValue(), () -> {
		    return new TreeMap<Long, TreeSet<String>>(Comparator.reverseOrder());
		}, Collectors.mapping(e -> e.getKey(), Collectors.toCollection(TreeSet::new))));
    }
	...
	return ordiniFornitore.get(codice).getLinee().stream()
		.collect(Collectors.summingInt(l -> l.getProdotto().getPrice())).toString();
    }
